...
Authorization is handled via a `PrivateRoute` component that accepts permitted roles and either redirects to the login page if the user is not authenticated, redirects to a forbidden page if the user is authenticated but their role is not whitelisted, or renders the provided component if the user is both authenticated and permitted to view that route. This is a pretty common routing pattern, but another option is to use a higher order component to handle view permissions. For example `withAuth([rolesEnum.Admin, rolesEnum.Author])(PostListPage)`.
...
There are a lot of ways to manage styling in react components. Some solutions include CSSinJS, styled-components, and CSS modules. In this project I have chosen to use css modules which allows each component to have its own scoped styles. I have also declared global css utility classes which compose components' local styles. Take for example:
...
.pageLoading {
  composes: fit-viewport-body from global;
  composes: children-center from global;
}
```
```jsx
/* PostListPage.js */
...
This allows use to reuse css utilities, while naming css classes semantically at the component level. By doing this we pretty much eliminate the need of the classNames utility because all of our composition can be done inside of our css modules instead of our js. CSS classes that we access in js we name in camel case, but global styles which we do not intend to include directly in our js we name with hyphens.
...
## Create React App Autogenerated Stuff
